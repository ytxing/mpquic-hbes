...
class MpExperience:
	...
	def runNetemAt(self):
		...
		for l in links:
			lname = self.mpConfig.getMidLeftName(i)
			rname = self.mpConfig.getMidRightName(i)
			cname = self.mpConfig.getLeftName()
			lbox = self.mpTopo.getHost(lname)
			cbox = self.mpTopo.getHost(cname)
			lif = self.mpConfig.getMidL2RInterface(i)
			lif1 = self.mpConfig.getMidL2RInterface1(i)
			rif = self.mpConfig.getMidR2LInterface(i)
			cif = self.mpConfig.getLeftInterface(i)
			rbox = self.mpTopo.getHost(rname)
			print(str(lname) + " " + str(lif))
			print(str(rname) + " " + str(rif))
			print("boxes " + str(lbox) + " " + str(rbox))
			...
			cmd = l.buildNetemdelayCmdx(lif1)
			print(cmd)
			self.mpTopo.commandTo(lbox, cmd)

			cmd = l.buildNetemdelayCmd(lif)
			print(cmd)
			self.mpTopo.commandTo(lbox, cmd)

			cmd = l.buildNetembandwidthCmdx(cif)
			print(cmd)
			self.mpTopo.commandTo(cbox, cmd)

			cmd = l.buildNetembandwidthCmd(rif)
			print(cmd)
			self.mpTopo.commandTo(rbox, cmd)
