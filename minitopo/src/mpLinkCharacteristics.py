...
class MpLinkCharacteristics:
	...
	def __init__(self, id, delay, jitter, queueSize, bandwidth, loss, back_up=False):
		self.id = id
		self.delay = delay
		#add jitter zy
		self.jitter = jitter
	...
	def buildNetemCmd(self, ifname):
		...
		for n in self.netemAt:
			...
			cmd = cmd + " netem " + n.cmd + " delay " + self.delay + "ms " + self.jitter + "ms && "
		cmd = cmd + " true &"
		return cmd
	def buildNetembandwidthCmd(self, ifname):
		cmd = ""
		for n in self.netemAt:
			cmd = cmd + "sleep 0.1" 
			cmd = cmd + " && tc qdisc del dev " + ifname + " root "
			cmd = cmd + " && tc qdisc add dev " + ifname + " root handle 1:0 tbf rate " + self.bandwidth
			cmd = cmd + "mbit burst " + str(int(self.queueSize) * 1500) + " latency " + self.queuingDelay
			cmd = cmd + "ms && tc qdisc show >> ping.log"
		# cmd = cmd + " true &"
		return cmd
	def buildNetembandwidthCmdx(self, ifname):
		cmd = ""
		for n in self.netemAt:
			cmd = cmd + "sleep 0.1" 
			cmd = cmd + " &&"
		...
		# cmd = cmd + " true &"
		return cmd
	def buildNetemdelayCmd(self, ifname):
		cmd = ""
		for n in self.netemAt:
			cmd = cmd + "sleep 0.1" 
			cmd = cmd + " && tc qdisc del dev " + ifname + " root "
			cmd = cmd + " && tc qdisc add dev " + ifname + " root netem " + n.cmd
			cmd = cmd + " delay " + self.delay + "ms " + self.jitter + "ms && tc qdisc show >> ping.log" 
			# + "ms && "
		# cmd = cmd + " true &"
		return cmd

	def buildNetemdelayCmdx(self, ifname):
		cmd = ""
		for n in self.netemAt:
			cmd = cmd + "sleep 0.1" 
			cmd = cmd + " &&"
			cmd = cmd + " tc qdisc add dev " + ifname + " root handle 1:0 netem " + n.cmd
			cmd = cmd + " delay " + self.delay + "ms " + self.jitter + "ms && tc qdisc show >> ping.log" 
			# + "ms && "
		# cmd = cmd + " true &"
		return cmd
