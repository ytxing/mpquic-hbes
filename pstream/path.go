...
func (p *path) setup(oliaSenders map[protocol.PathID]*congestion.OliaSender) {
	...
	p.bdwStats.RTTStats = p.rttStats
...
//ytxing: in bit per second
func (p *path) GetPathBandwidthByBytesAcked() protocol.ByteCount {

	utils.Debugf("ytxing: GetPathBandwidthByBytesCount() path %v: bandiwdth %v ", p.pathID, p.bdwStats.GetBandwidth()/1048576)
	return protocol.ByteCount(p.bdwStats.GetBandwidth()) //byte -> bit
