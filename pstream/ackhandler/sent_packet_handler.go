...
func (h *sentPacketHandler) SentPacket(packet *Packet) error {
	if packet.PacketNumber <= h.lastSentPacketNumber {
		// utils.Infof("ytxing: errPacketNumberNotIncreasing current %v <= last sent %v, but it is normal now", packet.PacketNumber, h.lastSentPacketNumber)
...
func (h *sentPacketHandler) ReceivedAck(ackFrame *wire.AckFrame, withPacketNumber protocol.PacketNumber, rcvTime time.Time) error {
	...
	if len(ackedPackets) > 0 {
		...
		for _, p := range ackedPackets {
			...
			// utils.Infof("UpdateBDW, Cwnd %v sentDelay %v", h.congestion.GetCongestionWindow(), sentDelay)
			// h.bdwStats.UpdateBDW(changeInflight, sentDelay)
			// utils.Infof("UpdateBDW, changeInflight %v sentDelay %v", h.congestion.GetCongestionWindow(), sentDelay)
		}
		...
		if flag == 1 {
			//utils.Infof("UpdateBDW, changeInflight %v sentDelay %v", changeInflight, sentDelay)
			h.bdwStats.UpdateBDW(changeInflight, sentDelay)
		}
...
//zy modified rtt expire time to SRTT,every window changed
func (h *sentPacketHandler) maybeUpdateRTT(largestAcked protocol.PacketNumber, ackDelay time.Duration, rcvTime time.Time) bool {
	var initialpktnum protocol.PacketNumber
	initialflag := 1
	if h.rttStats.RecentMinRTT() == 0 || h.rttStats.NumMinRTTsamplesRemaining() == 0 {
		h.rttStats.SetnumMinRTTsamplesRemaining(protocol.PacketNumber(h.congestion.GetCongestionWindow() / protocol.DefaultTCPMSS))
		h.rttStats.SettimefornumMinRTTsamplesRemaining(time.Now())
		utils.Debugf("zy:setnumminRTTsamplesremaining %v, time %v", h.rttStats.NumMinRTTsamplesRemaining(), time.Now())
	}
	if time.Since(h.rttStats.TimefornumMinRTTsamplesRemaining()) > (2 * h.rttStats.SmoothedRTT()) {
		h.rttStats.SetnumMinRTTsamplesRemaining(protocol.PacketNumber(h.congestion.GetCongestionWindow() / protocol.DefaultTCPMSS))
		h.rttStats.SettimefornumMinRTTsamplesRemaining(time.Now())
		utils.Debugf("zy:wait for two RTT, but no pkt received reset numMinRTTsamplesRemaining")
	}
	for el := h.packetHistory.Front(); el != nil; el = el.Next() {
		packet := el.Value
		if initialflag == 1 {
			initialpktnum = packet.PacketNumber
			initialflag = 0
		}
		utils.Debugf("zy: packetnum %v, packetlen %v", packet.PacketNumber, packet.Length)
		if packet.PacketNumber == largestAcked {
			pktnums := packet.PacketNumber - initialpktnum + 1
			samplesremaining := h.rttStats.NumMinRTTsamplesRemaining()
			if samplesremaining > pktnums {
				h.rttStats.SetnumMinRTTsamplesRemaining(samplesremaining - pktnums)
				//h.rttStats.UpdaterecentcwndMinRTT(rcvTime.Sub(packet.SendTime), ackDelay, true)
			} else {
				h.rttStats.SetnumMinRTTsamplesRemaining(protocol.PacketNumber(0))
				//h.rttStats.UpdaterecentcwndMinRTT(rcvTime.Sub(packet.SendTime), ackDelay, false)
				// utils.Debugf("zy:setnumminRTTsamplesremaining to 0 %v", h.rttStats.NumMinRTTsamplesRemaining())
			}

			// utils.Debugf("zy:maybeupdateRTT packetnum %v, this ack acked packetnum %v", packet.PacketNumber, pktnums)

			h.rttStats.UpdateRTT(rcvTime.Sub(packet.SendTime), ackDelay, time.Now())

			// if h.rttStats.NumMinRTTsamplesRemaining() == 0 {
			// 	if h.rttStats.LatestRTT() != 0 {
			// 		h.rttStats.SetRecentMinRTTwindow(h.rttStats.LatestRTT())
			// 	}
			// 	utils.Debugf("zy:recentminrttwindow %v", h.rttStats.RecentMinRTTwindow())
			// }
			return true
		}
...
func (h *sentPacketHandler) onPacketAcked(packetElement *PacketElement) {
	...
	//utils.Infof("ytxing: onPacketAcked path %v, h.bytesInFlight %v -= packetElement.Value.Length %v", h.pathID, h.bytesInFlight, packetElement.Value.Length)
...
func (h *sentPacketHandler) SendingAllowed() bool {
	...
	if congestionLimited {
		...
		utils.Debugf("Congestion not limited: Path %x, bytes in flight %d, window %d",
			h.pathID,
			h.bytesInFlight,
			h.congestion.GetCongestionWindow())
	}
