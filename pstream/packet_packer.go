...
func (p *packetPacker) StoreFrames(pth *path) (bool, error) {
...
func (p *packetPacker) PackPacket(pth *path) (*packedPacket, error) {
	...
	// Check if we have enough frames to send
	if len(payloadFrames) == 0 {
		utils.Debugf("len(payloadFrames) == 0")
		return nil, nil
	}
	// Don't send out packets that only contain a StopWaitingFrame
	if len(payloadFrames) == 1 && p.stopWaiting[pth.pathID] != nil {
		utils.Debugf("len(payloadFrames) == 1 && p.stopWaiting[pth.pathID] != nil")
...
func (p *packetPacker) composeNextPacket(
	...
	fs := p.streamFramer.PopStreamFrames(maxFrameSize - payloadLength) //ytxing: HERE!!!!!!!!!!!!!!!!!!
	if len(fs) != 0 {
...
func (p *packetPacker) getPublicHeader(encLevel protocol.EncryptionLevel, pth *path) *wire.PublicHeader {
	pnum := pth.packetNumberGenerator.Peek() //ytxing: only peek, not pop
...
func (p *packetPacker) writeAndSealPacket(
	...
	if buffer.Len()+sealer.Overhead()+2 > 1350 {
		utils.Infof("buffer.Len(%d)+sealer.Overhead(%d) %d < 1350", buffer.Len(), sealer.Overhead(), buffer.Len()+sealer.Overhead())
	}
