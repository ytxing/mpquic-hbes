...
func main() {
	...
	begin := time.Now()
	go func() {
		firstOrderURL := orderURLs[0]
		firstaddr := firstOrderURL.url
		firstprio := priorityURL[firstaddr].Weight
		firststart := time.Now()
		utils.Infof("GET %s, priority %d", firstaddr, firstprio)
		firstrsp, firsterr := hclient.Get(firstaddr)
		if firsterr != nil {
			panic(firsterr)
		}
		body := &bytes.Buffer{}
		_, firsterr = io.Copy(body, firstrsp.Body)
		if firsterr != nil {
			panic(firsterr)
		}
		firstelapsed := time.Since(firststart)
		utils.Infof("%s: %s", firstaddr, firstelapsed)
		wgfirst.Done()
	}()
	wgfirst.Wait()

	var wg sync.WaitGroup
	wg.Add(len(orderURLs) - 1)
	for x, orderURL := range orderURLs {
		if x >= 1 {
			addr := orderURL.url
