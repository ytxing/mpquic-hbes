...
type Download struct {
	ID           string
	Type         string // type of download files
	Started      bool
	Complete     bool
	StartTime    time.Time
	...
	if browser == "Safari" {
		for _, obj := range objs {
			if strings.Contains(obj.Download.Type, "html") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 5}
				...
				obj.PriorityParam = &http2.PriorityParam{Weight: 4}

			} else if strings.Contains(obj.Download.Type, "image") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 3}
	...
	for _, obj := range objs {
		for _, dep := range obj.Dependers {
			if id == dep.A2 {
				...
				// still have to wait dep.Time after the dependent stream was finished
...
func onCompletion(start time.Time) {
	...
	for _, obj := range objs {
		if obj.Download.Type == "text/html" || obj.Download.Type == "text/css" {
			if longest.Before(obj.Download.CompleteTime) {
				longest = obj.Download.CompleteTime
			}
		}
