...
func newStreamsMap(
	...
	connectionParameters handshake.ConnectionParametersManager) *streamsMap {
	sm := streamsMap{
		perspective:          pers,
		streams:              map[protocol.StreamID]*stream{},
...
func newStreamsMapPriority(newStreamPriority newStreamLambdaPriority,
	pers protocol.Perspective,
	connectionParameters handshake.ConnectionParametersManager) *streamsMap {
	sm := streamsMap{
		perspective:          pers,
...
func newStreamsMapPrioritySize(newStreamPrioritySize newStreamLambdaPrioritySize,
	pers protocol.Perspective,
	connectionParameters handshake.ConnectionParametersManager) *streamsMap {
	sm := streamsMap{
		perspective:           pers,
...
func (m *streamsMap) openStreamPriorityImpl(priority *protocol.Priority) (*stream, error) {
	...
	if m.streamScheduler != nil {
		err := m.streamScheduler.maybeSetWeight(id, priority.Weight)
		if err != nil {
			return nil, err
		}
		err = m.streamScheduler.maybeSetParent(id, priority.Dependency, priority.Exclusive)
		if err != nil {
			return nil, err
		}
	}
...
func (m *streamsMap) putStream(s *stream) error {
	...
	if m.streamScheduler != nil {
		...
		utils.Debugf("ytxing: addNode for stream %v", id)
	}
	return nil
}
