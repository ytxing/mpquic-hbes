...
type streamTree struct {
	...
	//toSend       *nodePstream
	openStreams  int
	nodeMap      map[protocol.StreamID]*nodePstream
...
//TODO, ytxing. Two exported methods need to be modified.

//ytxing: no called from outside this file
func newNode(id protocol.StreamID, stream *stream, parent *nodePstream) *nodePstream {
	return &nodePstream{
		id:      id,
		stream:  stream,
...
//ytxing: called by session.go
//
func newStreamTree() *streamTree {
	nodeMap := make(map[protocol.StreamID]*nodePstream)
...
func (n *nodePstream) deactivateNode() error {
	// if utils.Debug() {
...
func (n *nodePstream) setVisited() error {

	n.visited = true
	return nil
}

func (n *nodePstream) addWeight(child *nodePstream) {
	// if utils.Debug() {
	...
	utils.Debugf("ytxing: addWeight, add child  %v to parent %v, total activeChildren %v", child.id, n.id, n.activeChildren)
	if n.parent != nil && n.state != nodeActive && n.activeChildren == 1 {
		n.parent.addWeight(n)
	}
}

func (n *nodePstream) removeWeight(child *nodePstream) {
	// if utils.Debug() {
...
func (sch *streamTree) maybeSetParent(childID, parentID protocol.StreamID, exclusive bool) error {
	...
	if !ok {
		return fmt.Errorf("setting stream %d as exclusive 11111child of unknown stream %d", childID, parentID)
	}
...
//return all streams
func (sch *streamTree) traverseAll(n *nodePstream) (strm []*nodePstream) {
	// if utils.Debug() {
...
func (sch *streamTree) traverse(n *nodePstream) (strm []*nodePstream) {
	// if utils.Debug() {
...
func (sch *streamTree) printTree() {
	...
	var dfs func(*nodePstream, uint32, map[protocol.StreamID]uint32)

	dfs = func(n *nodePstream, level uint32, res map[protocol.StreamID]uint32) {
