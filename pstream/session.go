...
type session struct {
	...
	streamTree      *streamTree      //SHI
	streamScheduler *streamScheduler //ytxing
...
func (s *session) setup(
	...
	utils.Debugf("ytxing: session.go setup pathSchedulerDebug %v", pathSchedulerDebug)
	...
	s.streamsMap = newStreamsMapTree(s.newStreamPrioritySize, s.perspective, s.connectionParameters, s.streamTree, s.streamScheduler)
	s.streamFramer = newStreamFramerTree(s.streamsMap, s.flowControlManager, s.streamTree, s.streamScheduler)
...
func (s *session) handleFramesNew(fs []wire.Frame, p *path, localPconn net.PacketConn) error {
	for _, ff := range fs {
		...
		case *wire.StreamFrame:
			...
			if err == nil {
				utils.Debugf("ytxing: stream %d receive frame offset %v from path %v", frame.StreamID, frame.Offset, p.pathID)
			}
		case *wire.AckFrame:
...
func (s *session) handleStreamFrame(frame *wire.StreamFrame) error {
	...
	if frame.FinBit {
		...
		s.pathsLock.RLock()
		utils.Infof("frame.FinBit, Info for stream %d of %x", frame.StreamID, s.connectionID)
...
func (s *session) GetOrOpenStreamPrioritySize(id protocol.StreamID, priority *protocol.Priority) (Stream, error) {
	...
	stream := s.streamsMap.streams[id]
	if s.perspective == protocol.PerspectiveServer {
		if utils.Debug() {
			utils.Debugf("GetOrOpenStreamPrioritySize in Server: ID %v Size %v Weight %d, Dependency %d, Exclusive %t\n", id, stream.lenOfDataForWriting(), priority.Weight, priority.Dependency, priority.Exclusive)
		}
	}
	if str != nil {
		s.SetStreamPriority(id, priority)
...
func (s *session) SetStreamPriority(id protocol.StreamID, priority *protocol.Priority) error {
	if s.streamTree == nil || s.streamScheduler == nil {
		return nil
	}
	var err error
