...
//ytxing: same as sa-ecf, but add another tree
...
//ytxing: new with a streamScheduler also
func newStreamFramerTree(streamsMap *streamsMap, flowControlManager flowcontrol.FlowControlManager, streamTree *streamTree, streamScheduler *streamScheduler) *streamFramer {
	return &streamFramer{
		streamsMap:         streamsMap,
		flowControlManager: flowControlManager,
		streamTree:         streamTree,
...
//ytxing: from SE-ECF
func newStreamFramerScheduler(streamsMap *streamsMap, flowControlManager flowcontrol.FlowControlManager, streamScheduler *streamScheduler) *streamFramer {
	return &streamFramer{
		streamsMap:         streamsMap,
		flowControlManager: flowControlManager,
		streamScheduler:    streamScheduler,
...
func (f *streamFramer) maybePopNormalFrames(maxBytes protocol.ByteCount) (res []*wire.StreamFrame) {
	...
	//ytxing: streamScheduler.send is actually the streamLambda fn in maybePopNormalFrames but from one specific stream
	if pathSchedulerDebug == "saecf" || pathSchedulerDebug == "mine" || pathSchedulerDebug == "minrtt" {
		// if pathSchedulerDebug == "saecf" {
		utils.Debugf("pathSchedulerDebug %v, using modified method in stream_framer.go", pathSchedulerDebug)
		var s *node
		if f.streamScheduler.toSend == nil {
			if streamSchedulerDebug == "WRR" {
				s = f.streamScheduler.wrrSchedule() //ytxing: which stream to send
			} else if streamSchedulerDebug == "DFCFS" {
				s = f.streamScheduler.dynamicFCFSSchedule()
			} else if streamSchedulerDebug == "PRR" {
				s = f.streamScheduler.PRRSchedule()
			}
			if s == nil {
				utils.Debugf("ytxing: stream scheduler return stream nil")
			} else {
				utils.Debugf("ytxing: stream scheduler return stream %v", s.stream.streamID)
			}
		} else {
			s = f.streamScheduler.toSend
			if s == nil {
				utils.Debugf("ytxing: toSend stream nil")
			} else {
				utils.Debugf("ytxing: toSend stream %v", f.streamScheduler.toSend.stream.streamID)
			}
		}
		if s != nil && s.stream != nil {
			r, _, _ := f.streamScheduler.send(s.stream, maxBytes, nil)
			f.streamScheduler.toSend = nil
			if r != nil {
				res = append(res, r)
			}
		}
	...
	//ytxing: modification ends here
	if streamSchedulerDebug == "RR" {
		f.streamsMap.RoundRobinIterate(fn)
	} else if streamSchedulerDebug == "PRR" {
		f.streamScheduler.PriorityBasedRRScheduler(fn)
	}
